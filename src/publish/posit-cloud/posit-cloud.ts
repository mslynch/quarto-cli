...
export const kRSPositCloudServerVar = "POSIT_CLOUD_SERVER";
export const kRSConnectAuthTokenVar = "POSIT_CLOUD_TOKEN";
export const kRSPositCloudAuthTokenSecretVar = "POSIT_CLOUD_SECRET";
...
function getServer(): string {
  return Deno.env.get(kRSPositCloudServerVar) || "https://api.posit.cloud";
}
...
function accountTokens() {
  ...
  const server = Deno.env.get(kRSPositCloudServerVar);
  const token = Deno.env.get(kRSConnectAuthTokenVar);
  const tokenSecret = Deno.env.get(kRSPositCloudAuthTokenSecretVar);
...
async function authorizeToken(
  ...
  while (true) {
    const token = await Input.prompt({
      message: "Token:",
      hint: "Learn more at https://posit.cloud/learn/guide#account-tokens",
    });
    if (token.length === 0) {
      throw new Error();
    }
...
async function publish(
  ...
  try {
    ...
    const bundle = await withSpinner({
      ...
      const bundleHashBuffer = await crypto.subtle.digest(
        "MD5",
        bundleBytes,
      );
      const bundleHash = Array.from(new Uint8Array(bundleHashBuffer))
